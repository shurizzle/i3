...
/* Store the state of moving mouse: true if we need to move mouse, false if not */
static bool move_mouse = true;
...
void x_center_mouse_on_focused(void) {
    if (!config.disable_mouse_follows_focus) {
        Rect *warp_to = &focused->rect;
        int mid_x = warp_to->x + (warp_to->width / 2);
        int mid_y = warp_to->y + (warp_to->height / 2);
